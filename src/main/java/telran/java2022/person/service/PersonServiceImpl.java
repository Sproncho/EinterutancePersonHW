...
public class PersonServiceImpl implements PersonService, CommandLineRunner {
	...
	public Iterable<PersonDto> findEmployeeBySalary(int min, int max) {
		List<PersonDto> res = personRepository.findPersonsBySalary(min,max).map(p -> modelMapper.map(p,PersonDto.class)).collect(Collectors.toList());
		return res;
	}
	...
	public Iterable<PersonDto> getChildren() {
		return personRepository.findAllChildren().map(p -> modelMapper.map(p, PersonDto.class)).collect(Collectors.toList());
	}
